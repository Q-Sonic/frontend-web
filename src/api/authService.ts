...
export async function forgotPassword(email: string): Promise<ApiResponse<unknown>> {
  return api<ApiResponse<unknown>>('auth/forgot-password', {
    method: 'POST',
    body: JSON.stringify({ email }),
  });
}

export async function verifyResetCode(email: string, code: string): Promise<ApiResponse<unknown>> {
  return api<ApiResponse<unknown>>('auth/verify-reset-code', {
    method: 'POST',
...
export async function resetPassword(
  ...
  return api<ApiResponse<unknown>>('auth/reset-password', {
    method: 'POST',
